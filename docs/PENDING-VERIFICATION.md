...
| `shareholders` | PARTLY FIXED. The token endpoint answers `{"message":"Successfully retrieved Token","data":{"value":"<64 hex>"}}` — the token sits under `value`, which no `/token/i` key search can find, and `message` is the only string that mentions the word. Reading `data.value` on this one route settles the mint. The chart call behind it then fails differently: `rpc error: code = Unauthenticated desc = WebViewToken.FromContext: User Not Found`. **Narrowed 2026-09-01:** that same 401 comes back identically with a valid minted token, with no `token` parameter, and with `token=notarealtoken` — so the query parameter is not what the endpoint reads and no VALUE will fix it. The remaining candidates are a header and a POST body; settling between them needs a DevTools capture of Stockbit's own request on the ownership view, which the server cannot make of itself. Tool stays `projected` and now explains this in its own error. |
