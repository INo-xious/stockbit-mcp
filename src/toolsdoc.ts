...
import { byFamily, describeSurface, dominantEvidence } from "./tools/surface.js";
...
export async function renderToolsDoc(): Promise<string> {
  ...
  try {
    ...
    const grouped = byFamily(rows);
    const present = FAMILIES.filter((family) => grouped.has(family));
    ...
    for (const family of present) {
      const inFamily = grouped.get(family) ?? [];
      const dominant = dominantEvidence(inFamily);
      const evidence = dominant === "mixed" ? "Mixed" : EVIDENCE_LABEL[dominant];
      out.push(
